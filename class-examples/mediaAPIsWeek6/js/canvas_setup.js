window.onload = function () {
    ...
    context.lineWidth = 2; //change stroke
    context.stroke();//set the stroke
    context.closePath(); //begin and end path tell when the shape starts and ends = 2 seperate entitites = always except when rects

    //SECOND
    context.strokeStyle = "#0000FF"; // change the color we are using
    context.beginPath();
    context.arc(xPos + 200, yPos, radius, startAngle, endAngle, true);
    context.fill(); // set the fill after the arc = order is important 
    context.lineWidth = 2; //change stroke
    context.stroke();//set the stroke
    context.closePath();
